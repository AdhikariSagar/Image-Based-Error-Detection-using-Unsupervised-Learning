...
class Trainer():
    def __init__(self, model: object, hyperParams, DataLoderParams):
        ...
        self.image_size = hyperParams.get('IMAGE_SIZE')
        ...
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, factor=0.01, patience=5, mode='min', verbose=True, threshold=0.0001)
        ...
        self.file_name = 'cae_weights.pth'
    ...
    def train(self):
        ...
        for i_epoch in range(self.n_epoch):
            ...
            for features in self.train_dataLoader:
                ...
                loss = self.pixelwise_mse_loss(reconstruction, features)
            ...
            if (i_epoch+1)%5==0:
                self.plot_reconstruction(features, reconstruction)
            ...
            for features  in self.valid_dataLoader:
                with torch.no_grad():
                    features = features.cuda()
                    #print('in', img.shape)
                    reconstruction_val = self.model(features)
                    #print('out', reconstruction_val.shape)
                    val_loss = self.pixelwise_mse_loss(reconstruction_val, features)

                    epoch_loss_val += float(val_loss)
                    m += 1
    ...
    def compute_train_outputs(self):
            ...
            with torch.no_grad():
                ...
                for img in batch_imgs:
                    with torch.no_grad():
                        img = img[None, :, :, :]
                        img = img.cuda()
                        reconstruction_img = self.model(img)
                        difference = self.pixelwise_mse_loss(reconstruction_img.cuda(), img)
        ...
        print(f'Mean traing recon. error: {self.train_data["mean_train_error"]}')
    ...
    def show_error_distribution(self):
        ...
        fig = plt.figure()
        plt.fill_between(train_bins[:-1], train_counts, step="pre", color='green', alpha=0.4)
        plt.fill_between(test_bins[:-1], test_counts, step="pre", color='red', alpha=0.4)

        plt.step(train_bins[:-1],train_counts, drawstyle="steps", color='green', label='Undefected', fillstyle='full', alpha=.6)
        plt.step(test_bins[:-1], test_counts, drawstyle="steps", color='red', label='Defected', fillstyle='full', alpha=.6)
    ...
    def save_model(self, version="v1"):
        file_name = f'con_autoencoder_weights_{self.version}.pth'
        torch.save(self.model.state_dict(), self.save_dir / file_name)
        
        csv_name =  f'con_autoencoder_results_{self.version}.csv'
    ...
    def load_model(self, version="v1"):
        file_name = f'con_autoencoder_weights_{version}.pth'
